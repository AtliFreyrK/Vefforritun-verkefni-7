...
function start() {
  play();
let r = confirm("Viltu spila annan leik?");
if (r == true) {
  start();
} else {
...
function alert1(alertinput) {
  return window.alert(alertinput);
}
...
function play() {
  ...
  inputstring = 0;
  counterguesses = 0;
  while (inputstring != random && inputstring != null) {
    let inputstring = prompt("Giskaðu á tölu sem er á milli 0 og 100");
    if (inputstring == null) {
      window.alert("Hætt í leik");
      break;}
    let guess = parseGuess(inputstring);
    console.log(guess);
    let response = getResponse(guess, random);
    console.log(response);
    alert1(response);
    counterguesses++;
    console.log(counterguesses);
    if (inputstring == random) {
       countergames++;
      games[countergames-1]=counterguesses;
...
function getResponse(guess, correct) {
  let tala = Math.abs(correct - guess);
  b = Number(tala)
  if (guess< 0) {
    a = "Ekki rétt";
  }
